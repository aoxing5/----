...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    try {
        ...
        // 更新数据库
        $sql = "UPDATE book SET BookName=?, Author=?, CategoryID=?, Price=?, Stock=?, Description=?, ImageURL=? 
                WHERE BookID=?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("ssiidssi", $bookName, $author, $categoryId, $price, $stock, $description, $imageUrl, $bookId);
                ...
                <form method="POST" enctype="multipart/form-data">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="bookName" name="bookName" 
                               value="<?php echo htmlspecialchars($book['BookName']); ?>" required>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" class="form-control" id="stock" name="stock" 
                               value="<?php echo $book['Stock']; ?>" min="0" required>
                    </div>
                        ...
                        <?php if ($book['ImageURL']): ?>
                            <div class="mb-2">
                                <img src="<?php echo BASE_URL . $book['ImageURL']; ?>" 
                                     alt="当前封面" style="max-height: 100px;">
                            </div>
                        <?php endif; ?>
